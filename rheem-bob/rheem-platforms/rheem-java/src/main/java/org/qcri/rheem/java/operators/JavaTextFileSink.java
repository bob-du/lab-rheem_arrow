...
import org.qcri.rheem.java.channels.ArrowChannel;
...
//        JavaChannelInstance input = (JavaChannelInstance) inputs[0];
        ArrowChannel input =(ArrowChannel) inputs[0];
        ...
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(fs.create(this.textFileUrl)))) {
            input.provideVectorSchemaRoot().getFieldVectors().forEach(
                    dataQuantum -> {
                        try {
                            writer.write(formatter.apply((T) dataQuantum));
                            writer.write('\n');
                        } catch (IOException e) {
