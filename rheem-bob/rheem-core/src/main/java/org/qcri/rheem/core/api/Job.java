...
public class Job extends OneTimeExecutable {
     ...
     * @param experiment an {@link Experiment} for that profiling entries will be created    将为该分析条目创建一个{@link实验}
    ...
    Job(RheemContext rheemContext, String name, Monitor monitor, RheemPlan rheemPlan, Experiment experiment, String... udfJars) {
        ...
        for (String udfJar : udfJars) {//用户自定义jar
            this.addUdfJar(udfJar);
        }

        // Prepare re-optimization. 准备重新优化
    ...
    protected void doExecute() {
        ...
        try {
            ...
            this.prepareRheemPlan();
            // Estimate cardinalities and execution times for the #rheemPlan.估计#rheemPlan的基数和执行时间
            this.estimateKeyFigures();
    ...
    /**
     * 准备{@link #rheemPlan}:删除未使用的{@link操作符}，隔离循环，并应用所有可用的
    ...
    private void prepareRheemPlan() {
        ...
        // Prepare the RheemPlan for the optimization. 为优化工作准备变更计划。
        ...
        // Apply the mappings to the plan to form a hyperplan. 将映射应用到计划以形成hyperplan。
    ...
    /**
     * Gather all available {@link PlanTransformation}s from the {@link #configuration}.从{@link #配置}中收集所有可用的{@link PlanTransformation}
     */
    ...
    /**
     * 从给定的{@link RheemPlan}中确定一个好的/最好的执行计划。
    ...
    private ExecutionPlan createInitialExecutionPlan() {
        ...
        // Enumerate all possible plan.列举所有可能的计划。
        ...
        // Pick an execution plan.选择一个执行计划。
        // Make sure that an execution plan can be created.确保可以创建执行计划。
    ...
    /**
     *遍历给定的{@link RheemPlan}并更新在其之间传递的数据基数{@link Operator}使用给定的{@link ExecutionState}。
     *
    ...
    /**
     * 为{@link #rheemPlan}和{@link #configuration}创建一个新的{@link PlanEnumerator}。
     *
    ...
    /**
     * 使用所有附加功能开始执行给定的{@link ExecutionPlan}，例如检测、计划日志记录和测量执行时间。
     *
    ...
    private boolean execute(ExecutionPlan executionPlan, int executionId) {
        ...
        // Ensure existence of the #crossPlatformExecutor.确保#crossPlatformExecutor的存在。
